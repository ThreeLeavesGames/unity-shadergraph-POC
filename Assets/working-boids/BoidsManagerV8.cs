...
public struct BoidUpdateJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        // Anti-boundary
        boundaryForce += CalculateBoundaryForce(pos2D, parameters.perceptionRadius, antiBoundaryPoints);
...
[BurstCompile]
public struct BoidUpdateV8 : IJobParallelFor
{
    [ReadOnly] public NativeArray<float2> boundaryPoints;
    [ReadOnly] public NativeArray<float2> antiBoundaryPoints;  
...
public void Execute(int index)
{
    ...
    if (neighborCount > 0)
    {
        cohesion = (cohesion / neighborCount - position) * parameters.cohesionWeight;
        separation = separation * parameters.separationWeight;
        alignment = (alignment / neighborCount) * parameters.alignmentWeight;
    }

    // if (parameters.isSoloBoid)
    // {
    ...
    //         if (sqrDst < parameters.perceptionRadius * parameters.perceptionRadius * 0.5f)
    //         {
    ...
    // if (parameters.chaseWeight > 0)
    // {
    //     // Find closest prey
    //     for (int i = 0; i < currentPositions.Length; i++)
    //     {
    //         if ( hierarchy > hierarchyArray[i])
    //         {
    //             float3 offset = currentPositions[i] - position;
    //             float sqrDst = math.lengthsq(offset);
    //
    //             if (sqrDst < closestPreyDistance && sqrDst < parameters.perceptionRadius * parameters.perceptionRadius)
    //             {
    //                 closestPreyDistance = sqrDst;
    //                 preyDirection = math.normalize(offset);
    //             }
    ...
    for (int i = 0; i < currentPositions.Length; i++)
    {
        if (hierarchyArray[i] > hierarchy)
        {
            ...
            if (sqrDst < parameters.perceptionRadius * parameters.perceptionRadius * 4)
            {
                avoidPredator += math.normalize(offset) * (parameters.avoidPredatorWeight / math.sqrt(sqrDst));
            }
        }
    ...
    float2 pos2D = new float2(position.x, position.z);
    float avoidanceRadius = parameters.perceptionRadius * 2; // Distance at which to start avoiding boundaries
    ...
    float avoidanceRadiusAnti = parameters.perceptionRadius; // Larger radius for anti-boundary
    ...
    // if (parameters.chaseWeight > 0 && closestPreyDistance < float.MaxValue)
    // {
    //     velocity += preyDirection * parameters.chaseWeight;
    // }
    // Combine all forces
    ...
    velocity.y = 0;
    velocity = math.normalize(velocity) * parameters.speed;
