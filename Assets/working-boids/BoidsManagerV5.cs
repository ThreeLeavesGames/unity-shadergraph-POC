...
public class BoidsManagerV5 : MonoBehaviour
{
    ...
    void SpawnBoids()
    {
        ...
        for (int i = 0; i < predatorCount; i++)
        {
            ...
            predatorMatrices[i] = Matrix4x4.TRS(randomPos,  UnityEngine.Random.rotation, Vector3.one * 6);
        }
    ...
    void UpdateBoidsPositions()
    {
        ...
        PreyUpdateJobV5 preyJob = new PreyUpdateJobV5
        {
        ...
        PredatorUpdateJobV5 predatorJob = new PredatorUpdateJobV5
        {
            deltaTime = Time.deltaTime,
    ...
    void UpdateTransforms()
    {
        for (int i = 0; i < preyCount; i++)
        {
            ...
            // float3 velocity = preyVelocities[i];
            // if (math.lengthsq(velocity) > 0.001f)
            // {
            //     preyMatrices[i] = Matrix4x4.TRS(
            //         preyPositions[i], 
        ...
        for (int i = 0; i < predatorCount; i++)
        {
            float3 velocity = predatorVelocities[i];
            if (math.lengthsq(velocity) > 0.001f)
            {
                predatorMatrices[i] = Matrix4x4.TRS(
                    predatorPositions[i],
                    Quaternion.LookRotation(new Vector3(velocity.x, velocity.y, velocity.z)),
                    Vector3.one * 6
                );
        ...
        Graphics.DrawMeshInstanced(boidMesh, 0, boidMaterial, preyMatrices);
...
[BurstCompile]
public struct PreyUpdateJobV5 : IJobParallelFor
{
...
public void Execute(int index)
{
    ...
    // Combine all forces
    velocity += cohesion + separation + alignment + avoidPredator + avoidBoundary + antiBoundaryForce;
    ...
    //
    //     // Add extra separation force from neighbors in invalid positions
    //     float3 pushForce = float3.zero;
    //     for (int i = 0; i < currentPositions.Length; i++)
    //     {
    //         if (i == index) continue;
    //
    //         float2 neighborPos2D = new float2(currentPositions[i].x, currentPositions[i].z);
    //         if (IsPointInPolygon(neighborPos2D, antiBoundaryPoints))
    //         {
    ...
    //             if (sqrDst < perceptionRadius * perceptionRadius)
    //             {
    //                 pushForce += math.normalize(offset) * (separationWeight / math.sqrt(sqrDst));
    //             }
    //         }
    //     }
    //
    //     // Combine boundary avoidance with neighbor push
    //     velocity += turnForce + pushForce;
    //     velocity = math.normalize(velocity) * speed;
    //     velocity.y = 0;
    // }
...
[BurstCompile]
public struct PredatorUpdateJobV5 : IJobParallelFor
{
    [ReadOnly] public NativeArray<float2> boundaryPoints;
    // Time and position/velocity data
    public float deltaTime;                                    // Time since last frame
