...
public class BoidsManagerV3 : MonoBehaviour
{
    ...
    void UpdateBoidsPositions()
    {
        ...
        PreyUpdateJobV3 preyJob = new PreyUpdateJobV3
        {
        ...
        PredatorUpdateJobV3 predatorJob = new PredatorUpdateJobV3
        {
            deltaTime = Time.deltaTime,
...
[BurstCompile]
public struct PreyUpdateJobV3 : IJobParallelFor
{
    ...
    public void Execute_old(int index)
    {
        ...
        if (IsPointInPolygon(new float2(nextPosition.x, nextPosition.z)))
        {
            // If inside, move normally
            position = nextPosition;
        } else
        {
            // If outside, reflect velocity and stay at current position
            float2 currentPos2D = new float2(position.x, position.z);
    
            // Find closest boundary point
            float2 closestPoint = boundaryPoints[0];
            float minDist = float.MaxValue;
    
            for (int i = 0; i < boundaryPoints.Length; i++)
            {
                float2 point = boundaryPoints[i];
...
private bool IsPointInPolygon(float2 point)
    {
        ...
        for (int i = 0; i < boundaryPoints.Length; i++)
        {
            float2 current = boundaryPoints[i];
            float2 next = boundaryPoints[(i + 1) % boundaryPoints.Length];
...
[BurstCompile]
public struct PredatorUpdateJobV3 : IJobParallelFor
{
    [ReadOnly] public NativeArray<float2> boundaryPoints;
    // Time and position/velocity data
    public float deltaTime;                                    // Time since last frame
